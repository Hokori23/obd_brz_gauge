...
if ($homeRuntime -notmatch '(?s)static void ui_home_pager_commit_handler\(const ui_home_pager_commit_t \*commit,\s*void \*user\).*ui_home_refresh_timer_apply_profile\(commit->to_page\);.*aux_sensor_demand_refresh\(\);') {
